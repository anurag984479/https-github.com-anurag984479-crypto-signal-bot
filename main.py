...
# PATCH: ULTIMATE WINRATE UPGRADE v2026.5
...
# HYBRID SCALP SETTINGS — v2026.5 PATCHED
...
# SCORE THRESHOLDS — v2026.5 PATCHED
...
# SESSION CURATION — v2026.5 PATCHED (Asian removed)
...
# DAILY SIGNAL CAPS — v2026.5 PATCHED (reduced overtrading)
...
return False
return price > weekly_ma if direction == “BUY” else price < weekly_ma
...
if pd.isna(h12_ma):
return False
return price > h12_ma if direction == “BUY” else price < h12_ma
...
# SMART AI SHORT TERM TREND — v2026.5 NEW
...
if ema9 > ema21 and aox > 0 and wt1 > wt2:
    return "BULLISH"
elif ema9 < ema21 and aox < 0 and wt1 < wt2:
    return "BEARISH"
...
# ORDER BLOCK RECLAIM — v2026.5 NEW

# ============================================================

def order_block_reclaim(df, direction):
if len(df) < 8:
return False
...
if direction == "BUY":
    ...
    return float(df.iloc[-1]["close"]) > ob_low

elif direction == "SELL":
    ob_high = float(recent["high"].max())
    return float(df.iloc[-1]["close"]) < ob_high
...
# FAIR VALUE GAP ENTRY — v2026.5 NEW

# ============================================================

def imbalance_entry(df, direction):
if len(df) < 3:
return False
...
# ELITE STRONG CANDLE — v2026.5 NEW (strict 70% body)
...
if direction == "BUY":
    return (
        close_p > open_p and
        close_p >= high_p - total_range * 0.15
    )
elif direction == "SELL":
    return (
        close_p < open_p and
        close_p <= low_p + total_range * 0.15
    )
...
# SCALP SIGNAL ENGINE — ELITE MICRO-SNIPER v2026.5
...
support_touch = (
    float(last["low"]) <= float(recent_lows.min()) * 1.001
)

resistance_touch = (
    float(last["high"]) >= float(recent_highs.max()) * 0.999
)
...
if SCALP_ADX_MIN <= adx <= SCALP_ADX_MAX:
    ...
    if smart_trend == "BULLISH":
        buy_score += 4
        buy_cond["SMART_AI_BULLISH"] = True
    ...
    if order_block_reclaim(df, "BUY"):
        buy_score += 4
        buy_cond["OB_RECLAIM"] = True

    if imbalance_entry(df, "BUY"):
        buy_score += 3
        buy_cond["FVG_IMBALANCE"] = True

    if elite_strong_candle(df, "BUY"):
        buy_score += 4
        buy_cond["STRONG_CANDLE"] = True
...
if SCALP_ADX_MIN <= adx <= SCALP_ADX_MAX:
    ...
    if smart_trend == "BEARISH":
        sell_score += 4
        sell_cond["SMART_AI_BEARISH"] = True
    ...
    if micro_bos(df, "SELL"):
        sell_score += 4
        sell_cond["MICRO_BOS"] = True

    if order_block_reclaim(df, "SELL"):
        sell_score += 4
        sell_cond["OB_RECLAIM"] = True

    if imbalance_entry(df, "SELL"):
        sell_score += 3
        sell_cond["FVG_IMBALANCE"] = True

    if elite_strong_candle(df, "SELL"):
        sell_score += 4
        sell_cond["STRONG_CANDLE"] = True
...
if   score >= 34: return “BLACKROCK-TIER”
...
# HYBRID MASTER SIGNAL ENGINE — v2026.5 PATCHED
...
if regime == "RANGE":
    ...
    if best_scalp >= SCALP_MIN_SCORE:
        ...
        # =====================================================
        # ELITE REVERSAL CLUSTER BONUS — v2026.5
        # =====================================================
        if direction == "BUY":
            ...
            if (
                ...
                "OB_RECLAIM" in scalp_buy_cond
            ):
                best_scalp += 3

            if (
                "SMART_AI_BULLISH" in scalp_buy_cond and
                "FVG_IMBALANCE" in scalp_buy_cond
            ):
                best_scalp += 4

            if (
                "STRONG_CANDLE" in scalp_buy_cond and
                "LIQUIDITY_SWEEP" in scalp_buy_cond
            ):
                best_scalp += 4
            ...
            if (
                ...
                "OB_RECLAIM" in scalp_sell_cond
            ):
                best_scalp += 3

            if (
                "SMART_AI_BEARISH" in scalp_sell_cond and
                "FVG_IMBALANCE" in scalp_sell_cond
            ):
                best_scalp += 4

            if (
                "STRONG_CANDLE" in scalp_sell_cond and
                "LIQUIDITY_SWEEP" in scalp_sell_cond
            ):
                best_scalp += 4
...
if mode == "SCALP":
    cond_text += "\n⚡ SMART AI MICRO-SNIPER REVERSAL"
...
f”🧠 Wizard AI Active\n”
f”🛡 Smart AI Micro-Sniper Active\n”
f”⚡ OB Reclaim + FVG Imbalance Engine\n”
f”🧵 Thread Safe\n”
f”⚡ ULTIMATE HYBRID SUPREME 2026 ELITE — WINRATE UPGRADE v2026.5 LIVE”
)
